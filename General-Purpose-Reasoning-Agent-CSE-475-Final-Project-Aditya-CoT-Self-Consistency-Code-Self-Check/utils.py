...
Concurrency model:
  - The per-question call counter is a `contextvars.ContextVar` holding a
    one-element list (a mutable cell). When dev_eval runs questions in
    parallel via a ThreadPoolExecutor, each worker thread gets its own
    counter; sub-pools created by parallel.call_llm_concurrent inherit the
    parent question's cell via copy_context() so all sub-calls increment
    the SAME counter. This lets us safely run multiple questions at once
    without conflating their per-question budgets.

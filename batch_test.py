...
def _run_batch(
    ...
    print(f"\n{'=' * 78}")
    print(f"{label}")
    print(f"  {len(items)} questions x ~{expected_calls} calls = ~{len(items) * expected_calls} calls expected")
    print('=' * 78)
...
def main() -> int:
    ...
    print(f"\nPlanned run:")
    print(f"  common_sense       : {args.cs}  questions x 5 calls = {args.cs * 5} calls")
    print(f"  future_prediction  : {args.fp}  questions x 4 calls = {args.fp * 4} calls")
